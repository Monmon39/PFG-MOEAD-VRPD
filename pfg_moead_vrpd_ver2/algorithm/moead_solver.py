...
class MOEADSolver:
    ...
    def _init_population(self):
        ...
        for _ in range(self.pop_size):
            ...
            # === DRONE INITIALIZATION (distance-aware, đúng VRP-D)
            for route in sol.truckRoutes:
                ...
                for i in range(1, len(route) - 1):
                    ...
                    truck_dist = (
                        Evaluator._dist(self.customers, prev, cid)
                        + Evaluator._dist(self.customers, cid, nxt)
                    )
                    drone_dist = truck_dist

                    if drone_dist < 0.7 * truck_dist and self.rnd.random() < 0.5:
                        dset.add(cid)
    ...
    # CORE
    def _choose_mating_pool(self, i: int):
        neigh = self.neighborhood[i]

        def pick():
            return (
                neigh[self.rnd.randrange(len(neigh))]
                if self.rnd.random() < self.delta
                else self.rnd.randrange(self.pop_size)
            )
    ...
    def _update_neighborhood(self, i: int, child: Solution):
        for idx in self.neighborhood[i]:
            ...
            # === ALLOW DRONE EXPLORATION (theo VRP-D)
            if (
                f_child < f_old
                or (
    ...
    def _swap_mutate(self, perm: List[int]):
        i, j = self.rnd.randrange(len(perm)), self.rnd.randrange(len(perm))
